		...
		if netns, err := rootlessutil.DetachedNetNS(); err != nil {
			...
			if netns != "" {
				...
				return helpers.Custom("nsenter", "-t", strconv.Itoa(childPid), "-U", "-m", "--preserve-credentials",
					"--", "sh", "-c", fmt.Sprintf("nsenter --net=%s sh -c 'iptables -t nat -S && iptables -t filter -S && iptables -t mangle -S'", netnsPath))
			}
			// Enter the user and mount namespace
			return helpers.Custom("nsenter", "-t", strconv.Itoa(childPid), "-U", "-m", "-n", "--preserve-credentials",
				"--", "sh", "-c", "iptables -t nat -S && iptables -t filter -S && iptables -t mangle -S")
		}
	}
